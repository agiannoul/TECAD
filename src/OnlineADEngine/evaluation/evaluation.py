...
def calculate_ts_recall(anomalyranges, predictionsforrecall):
    ...
    if os.name == 'nt':
        ...
        AD1 = float(
            subprocess.run(
                ['./src/OnlineADEngine/evaluation/evaluate', '-t', anomaly_ranges_file_name, prediction_file_name, '1', '1', 'one', 'flat', 'flat'],
                stdout=subprocess.PIPE,
                text=True)
                .stdout.split('\n')[1].split('=')[1].strip()
        )
        ...
        AD2 = float(
            subprocess.run(
                ['./src/OnlineADEngine/evaluation/evaluate', '-t', anomaly_ranges_file_name, prediction_file_name, '1', '0', 'one', 'flat', 'flat'],
                stdout=subprocess.PIPE,
                text=True)
                .stdout.split('\n')[1].split('=')[1].strip()
        )
        ...
        AD3 = float(
            subprocess.run(
                ['./src/OnlineADEngine/evaluation/evaluate', '-t', anomaly_ranges_file_name, prediction_file_name, '1', '1', 'one', 'back', 'back'],
                stdout=subprocess.PIPE,
                text=True)
                .stdout.split('\n')[1].split('=')[1].strip()
        )
