    ...
    * ED1 conciseness is returned as a single value, corresponding to the `global` key only.
    ...
    def compute_metrics(self, periods, periods_labels, periods_info, used_data, include_ed2=True):
        ...
        for type_, type_instances in instances_dict.items():
            ...
            #print(f"\n Type {type_}")
            #print(f"\t{explanations_dict[type_]}")
            #print(f"\tType {type_}: {sum([len(exp) for exp in explanations_dict[type_]])/len(explanations_dict[type_])}")
        ...
        if not include_ed2:
            ...
            return {k: v for k, v in ed_metrics.items() if 'ed2_conciseness' not in k},dict()
...
class ModelDependentEvaluator(EDEvaluator):
    ...
    def explain_instance(self, instance, instance_labels):
        ...
        if anomaly_length == sample_length:
            return self.explainer.explain_sample(instance[anomaly_start:anomaly_end],instance_labels[anomaly_start:anomaly_end])
        ...
        return self.explainer.explain_sample(instance[slice(*sample_range)],instance_labels[slice(*sample_range)])
        ...
        *"windows" are all of size `self.explainer.sample_length`.
        ...
        if instance_length == self.min_instance_length or self.large_anomalies_coverage == 'end':
            ...
            for _ in range(self.ed1_consistency_n_disturbances - 1):
                ...
                sample_explanation, _ = self.explainer.explain_sample(instance[start:(start + sample_length)],instance_labels[start:(start + sample_length)])
                samples_fts.append(sample_explanation['important_fts'])
                slide_sign *= -1
                if slide_sign == 1:
                    slide += 1
