...
def run_experiment_SKAB(dataset,methods, param_space_dict_per_method,method_names,experiments, experiment_names,MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1,profile_size=None):
...
def execute_SKAB_method(selected_method = "IF", MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1):

    print("script: SKAB/run_online.py")
    tracking_uri = mlflow.get_tracking_uri()
    print("MLflow Tracking URI:", tracking_uri)
    dataset = loadDataset.get_dataset("SKAB")
    experiments = [
    ...
    best_params=run_experiment_SKAB(dataset,methods, param_space_dict_per_method, method_names, experiments, experiment_names, MAX_RUNS, MAX_JOBS, INITIAL_RANDOM,profile_size=profile_size)
    return best_params
...
def run_experiment_combinator(Classifier="xgb",selected_method = "IF",detector_weight=None):
    ...
    best_param=execute_SKAB_method(selected_method=selected_method, MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1)
    ...
    execute_SKAB_combinator(Classifier = Classifier, detector_weight = detector_weight,selected_method =selected_method,method_class=method_class,method_parameters=method_parameters,method_th=method_th,profile_size=profile_size)

def execute_SKAB_combinator(Classifier = "xgb", detector_weight = None, selected_method = "IF", method_class=IsolationForest,
    ...
    run_experiment_SKAB(dataset,methods, param_space_dict_per_method, method_names, experiments, experiment_names, MAX_RUNS, MAX_JOBS, INITIAL_RANDOM,profile_size=profile_size)


def execute_SKAB_cassifier_raw(Classifier = "xgb",rs=123, MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1):
    ...
    method_names = [
        f'Classifier {Classifier} Raw'
    ]
    methods = [
        Classifier_raw,
    ]
    run_experiment_SKAB(dataset,methods, param_space_dict_per_method, method_names, experiments, experiment_names, MAX_RUNS, MAX_JOBS, INITIAL_RANDOM)
...
def main():
    ...
    if args.experiment == "classifier":
        if not args.Classifier:
            ...
            return
        execute_SKAB_cassifier_raw(Classifier=args.Classifier)
        ...
        if not args.selected_method:
            ...
            return
        execute_SKAB_method(selected_method=args.selected_method)
