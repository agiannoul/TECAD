...
def run_experiment_BATDAL(dataset,methods, param_space_dict_per_method,method_names,experiments, experiment_names,MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1,profile_size=None):
...
def execute_BATADAL_combinator(Classifier = "xgb", detector_weight = None, selected_method = "IF", method_class=IsolationForest,
    ...
    print("script: BATADAL/run_online.py")
    tracking_uri = mlflow.get_tracking_uri()
    print("MLflow Tracking URI:", tracking_uri)
    dataset = loadDataset.get_dataset("BATADAL")
    experiments = [
    ...
    return run_experiment_BATDAL(dataset,methods, param_space_dict_per_method, method_names, experiments, experiment_names, MAX_RUNS, MAX_JOBS, INITIAL_RANDOM,profile_size=profile_size)


def execute_BATADAL_cassifier_raw(Classifier = "xgb",rs=123, MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1):
    ...
    run_experiment_BATDAL(dataset,methods, param_space_dict_per_method, method_names, experiments, experiment_names, MAX_RUNS, MAX_JOBS, INITIAL_RANDOM)


def execute_BATADAL_method(selected_method = "IF", MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1):
    ...
    allmethod_param_list = [
        {'k': [10], 'window_norm': [False]},
        {'n_estimators': [200], 'max_samples': [200], 'random_state': [42], 'max_features': [0.8], 'bootstrap': [True]},
        {'kernel': ['sigmoid'], 'nu': [0.3], 'gamma': ['scale']},
        {'window_size': [10]},
    ]
...
def run_experiment_combinator(Classifier="xgb",selected_method = "IF",detector_weight=None):
    ...
    profiles=[None,None,None,None,None]

    best_param=execute_BATADAL_method(selected_method=selected_method,MAX_RUNS=1, MAX_JOBS=1, INITIAL_RANDOM=1)

    pos = allmethodnames.index(selected_method)
    method_class = allmethodclasses[pos]
    method_parameters = allmethod_param_list[pos]
    method_th = best_param['th']
    print(method_th)
    profile_size = profiles[pos]


    execute_BATADAL_combinator(Classifier = Classifier, detector_weight = detector_weight,selected_method =selected_method,method_class=method_class,method_parameters=method_parameters,method_th=method_th,profile_size=profile_size)
...
# execute_BATADAL_method("IF")
# execute_BATADAL_method("KNN")
# execute_BATADAL_method("OCSVM")
# execute_BATADAL_method("CNN")
# execute_BATADAL_cassifier_raw(Classifier= "xgb")
# execute_BATADAL_cassifier_raw(Classifier= "svc")

# run_experiment_combinator(Classifier="xgb",selected_method = "IF",detector_weight=None)
# run_experiment_combinator(Classifier="xgb",selected_method = "KNN",detector_weight=None)
# run_experiment_combinator(Classifier="xgb",selected_method = "CNN",detector_weight=None)
...
# run_experiment_combinator(Classifier="xgb",selected_method = "OCSVM",detector_weight=None)
...
def main():
    ...
    parser.add_argument("experiment", choices=["classifier", "combinator", "OAD"], help="Which experiment to run")
    ...
    if args.experiment == "classifier":
        if not args.Classifier:
            ...
            return
        execute_BATADAL_cassifier_raw(Classifier=args.Classifier)
        ...
        if not args.selected_method:
            ...
            return
        execute_BATADAL_method(selected_method=args.selected_method)
